...
function update_category(id) {
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.success == true) {
                $('#update_image').attr('src', result.data.image);
                $('#update_category_form').attr("action", base_url + "/admin/category/" + result.data.id);
                $('#update_category').val(result.data.name);
            }
        },
        error: function (err) {
            console.log('err ', err)
...
function change_zoner(url, id) {
    console.log(base_url + '/' + url + '/change_zoner');
    $.ajax({
        headers:
        {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        success: function (result) {
            iziToast.success({
                message: 'Change zoner successfully..!!',
                position: 'topRight',
            })
        },
        error: function (err) {
...
function change_storage_status(selectObject,id){
    var value = selectObject.value;  
  console.log(value);
  console.log(id);
  console.log(base_url + '/admin/storage/change_storage_status');
    $.ajax({
        headers:
        {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        type: "POST",
        url: base_url + '/admin/storage/change_storage_status',
        data:
        {
            id: id,
...
function change_warehouse_status(value,id){
