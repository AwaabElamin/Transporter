...
class LicenseBoxAPI{
	...
	public function __construct(){
		$this->product_id = 'A864746E';
		$this->api_url = 'https://licencefly.com/';
		$this->api_key = 'BB0F06049D92F0-mealup';
	...
	public function activate_license($license, $client, $create_lic = true){
		return array('status' => true, 'message' => 'Valid key //prowebber');
		$data_array =  array(
			"product_id"  => $this->product_id,
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["3d0366797b39d25"])){
				$_SESSION["3d0366797b39d25"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["3d0366797b39d25"])){
				$get_data = $this->call_api(
					'POST',
