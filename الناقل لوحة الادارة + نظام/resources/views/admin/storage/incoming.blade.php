@extends('layouts.app',['activePage' => 'incoming'])

@section('title','Warehouse incoming')

@section('content')

<section class="section">
    <div class="section-header">
        <h1>{{__('Warehouse incoming')}}</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="{{ url('admin/home') }}">{{__('Dashboard')}}</a></div>
            <div class="breadcrumb-item active"><a href="{{ url('admin/storage') }}">{{__('Inventory')}}</a></div>
            <div class="breadcrumb-item">{{__('Warehouse incoming')}}</div>
        </div>
    </div>
    <div class="section-body">
        <h2 class="section-title">{{__('storage page')}}</h2>
        <p class="section-lead">{{__('Warehouse incoming')}}</p>
        <div class="card">
            
                <div class="card-header">
                <h4>{{__('Warehouse incoming')}}</h4>
                    ...
                    <tbody>
                        @foreach ($orders as $order)
                            <tr>
                                ...
                                <th>
                                    <a href="javascript:void(0);" class="table-action btn btn-success btn-action" onclick="change_storage_status(this,{{$order->id}})">
