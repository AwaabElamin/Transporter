...
class UserApiController extends Controller
{
        ...
        if ($request->provider == 'LOCAL') {
                ...
                if ($user->status == 1) {
                    if ($user->roles->contains('title', 'user')) {
                        ...
                        if ($user['is_verified'] == 1) {
                            $user['token'] = $user->createToken('Alnaqel')->accessToken;
            ...
            if ($data['provider'] !== 'LOCAL') {
                ...
                if ($user) {
                    ...
                    $token = $user->createToken('Alnaqel')->accessToken;
                    $user->save();
                    $user['token'] = $token;
                    return response()->json(['success' => true, 'data' => $user], 200);
                } else {
                    ...
                    $data->save();
                    $token = $data->createToken('Alnaqel')->accessToken;
                    $data['token'] = $token;
    ...
    {
        $master = array();
        $master['vendor'] = Vendor::where([['id', $vendor_id], ['status', 1]])->first(['id', 'image', 'tax', 'name', 'map_address', 'sp','mp','lp', 'vendor_type','lat','lang', 'category_id'])->makeHidden(['vendor_logo']);
    ...
    public function apiCategory()
    {
        ...
        return response(['success' => true, 'data' => $categorys]);
    }
    ...
    {
        return Vendor::get();
        $vendors = Vendor::where('status', 1)->orderBy('id', 'DESC')->get(['id', 'image', 'name', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
    ...
    public function apiFaviroute(Request $request)
    {
        ...
        if ($data != null) {
            ...
            return response(['success' => false, 'data' => 'No Vendor found..!!']);
        }
    }
    ...
    public function apiNearBy(Request $request)
    {
        ...
        // $vendors = Vendor::where('status', 1)->get(['id', 'image', 'name', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
        $vendors = Vendor::where('status', 1)->GetByDistance($request->lat, $request->lang, $radius)->get(['id', 'image', 'name', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
        foreach ($vendors as $vendor) {
            if (auth('api')->user() != null) {
                $user = auth('api')->user();
                $vendor['like'] = in_array($vendor->id, explode(',', $user->faviroute));
    ...
    public function apiRestFaviroute(Request $request)
    {
        ...
        $vendors = Vendor::where('status', 1)->whereIn('id', $faviroute)->get(['id', 'name', 'image', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
        foreach ($vendors as $vendor) {
            $lat1 = $vendor->lat;
            $lon1 = $vendor->lang;
            $lat2 = $request->lat;
    ...
    public function apiFilter(Request $request)
    {
        ...
        if (isset($request->cousins)) {
            ...
            foreach ($vendors as $vendor) {
                $categoryId = explode(',', $vendor->category_id);
                if (($key = array_search($request->cousins, $categoryId)) !== false) {
        ...
        $data = $result->get(['id', 'name', 'image', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
    ...
    {
        $vendors = Vendor::where([['vendor_type', 'veg'], ['status', 1]])->orderBy('id', 'DESC')->get(['id', 'image', 'name', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
        foreach ($vendors as $vendor) {
            $lat1 = $vendor->lat;
            $lon1 = $vendor->lang;
            $lat2 = $request->lat;
    ...
    {
        $vendors = Vendor::where([['vendor_type', 'non_veg'], ['status', 1]])->orderBy('id', 'DESC')->get(['id', 'image', 'name', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
        foreach ($vendors as $vendor) {
            $lat1 = $vendor->lat;
            $lon1 = $vendor->lang;
            $lat2 = $request->lat;
    ...
    {
        $vendors = Vendor::where([['isTop', '1'], ['status', 1]])->orderBy('id', 'DESC')->get(['id', 'image', 'name', 'lat', 'lang', 'vendor_type', 'category_id'])->makeHidden(['vendor_logo']);
        foreach ($vendors as $vendor) {
            $lat1 = $vendor->lat;
            $lon1 = $vendor->lang;
            $lat2 = $request->lat;
    ...
    {
        $vendors = Vendor::where([['isExplorer', '1'], ['status', 1]])->orderBy('id', 'DESC')->get(['id', 'image', 'name', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
        foreach ($vendors as $vendor) {
            $lat1 = $vendor->lat;
            $lon1 = $vendor->lang;
            $lat2 = $request->lat;
    ...
    public function apiCategoryVendor($id)
    {
        $v = Vendor::where('status', 1)->get(['id', 'image', 'name', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
        $vendors = array();
        foreach ($v as $vendor) {
            $categorys = explode(',', $vendor->category_id);
            if (($key = array_search($id, $categorys)) !== false) {
    ...
    public function apiSearch(Request $request)
    {
        ...
        $req = [];
        $radius = GeneralSetting::first()->radius;
        $req['vendor'] = Vendor::where('status', 1)->get(['id', 'image', 'name', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
        // $req['vendor'] = Vendor::where('status', 1)->GetByDistance($request->lat, $request->lang, $radius)->get(['id', 'image', 'name', 'lat', 'lang', 'category_id', 'vendor_type'])->makeHidden(['vendor_logo']);
        // $req['vendor'] = Vendor::where('name','LIKE','%'.$data['name']."%")->GetByDistance($request->lat, $request->lang, $radius)->get(['id','image','name','lat','lang','category_id','vendor_type']);
        $req['category'] = Category::where('name', 'LIKE', '%' . $data['name'] . "%")->get(['id', 'name', 'image']);
    ...
    public function sendNotification($user)
    {
        ...
        if ($admin_verify_user == 1) {
            ...
            if ($user->language == 'العربية') {
                $msg_content = $verification_content->arabic_notification_content;
                $mail_content = $verification_content->arabic_mail_content;
    ...
    public function sendVendorOrderNotification($vendor, $order_id)
    {
        ...
        if ($vendor->vendor_language == 'العربية') {
            $detail['Vendor_name'] = $vendor->name;
            $detail['Order_id'] = $order_id;
            $detail['User_name'] = auth()->user()->name;
            $v = ["{Vendor_name}", "{Order_id}", "{User_name}"];
            $notification_content = str_replace($v, $detail, $content->arabic_notification_content);
            ...
            $mail = str_replace($v, $detail, $content->arabic_mail_content);
            if ($vendor_mail == 1) {
                try {
