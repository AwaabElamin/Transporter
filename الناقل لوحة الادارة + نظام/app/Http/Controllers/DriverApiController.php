...
class DriverApiController extends Controller
{
    ...
    public function apiSetLocation(Request $request)
    {
        ...
        {
            $id = auth()->user();
            $id->delivery_zone_id = $request->deliveryZoneId;
            $id->save();
            return response(['success' => true , 'data' => 'successfully Set this location'.$request->deliveryZoneId]);
        }
        else
        {
    ...
    public function apiDriverOrder()
    {
        ...
        $vendor_driver = $this->checkVendorDriver();
        // return response(['success' => true ,'ohmy'=>'ih', 'data' => '[]']);
        $driver = auth()->user();
        $temp = array();
        if($vendor_driver == 1)
        {
...
$sql = 'SELECT user_id, ( 3959 * acos( cos( radians(' . $delivery_zone_area->lat . ') ) * cos( radians( lat ) ) * cos( radians( lang ) - radians(' . $delivery_zone_area->lang . ') ) + sin( radians(' . $delivery_zone_area->lat . ') ) * sin( radians(lat) ) ) ) AS distance FROM user_address HAVING distance < ' . $delivery_zone_area->radius . ' ORDER BY distance';
                    $near_users = DB::select(DB::raw($sql));
                    foreach($near_users as $near_user)
                    {
                        array_push($temp,$near_user->user_id);
                        array_push($users,$near_user->user_id);
                    }
                }
            }
            $storage = Storage::whereIn('vendor_id',$vendors)->where('order_status','incoming')->get(['id','address_id','order_id','user_id','vendor_id','payment_status','address_id','order_status','amount','payment_type','delivery_type','special']);
             $orders = Order::whereIn('user_id',$users)->where('order_status','APPROVE')->get(['id','address_id','order_id','user_id','vendor_id','payment_status','address_id','order_status','amount','payment_type','delivery_type','special']);
            return response(['success' => true ,'storage'=>$storage, 'data' => $orders,]);
        }
        else
        {
            $orders = Order::where([['delivery_person_id',$driver->id],['order_status','APPROVE']])->get(['id','address_id','order_id','user_id','payment_status','address_id','order_status','vendor_id','amount','payment_type','special']);
            return response(['success' => true ,'ohmy'=>'hi', 'data' => $orders,'ff' => $near_vendors]);
        }
    }
    ...
    public function apiDriverSetting()
    {
        ...
        if (auth('driverApi')->user() != null)
        {
            ...
            $setting->zoner = $user['zoner'] == 1 ? 'true' : 'false';
        }
