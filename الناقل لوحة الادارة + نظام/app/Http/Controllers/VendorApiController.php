...
class VendorApiController extends Controller
{
    public function getUsers(Request $request){
        ...
        foreach ($users as $value) {
            if($value->roles->contains('title', 'user')){

            array_push($userslist,$value->makeHidden(['created_at','updated_at','menu_category_id','vendor_id','email_verified_at','device_token','otp','roles','account_number','account_name','provider_token','provider_type']));
            }
        }
        return response(['success' => true , 'data' => $userslist]);
    }
    ...
    public function apiRegister(Request $request)
    {
        ...
        $cusinies = Category::whereStatus(1)->get('id');
        $tempIds = [];
        foreach ($cusinies as $value) {
            array_push($tempIds,$value->id);
        }
        $category_id = implode(',',$tempIds);
    ...
    {
        $vendor = Vendor::where('user_id', auth()->user()->id)->first();
        $storage = Storage::where('vendor_id', $vendor->id)->get()->each->setAppends(['orderItems'])->makeHidden(['created_at', 'updated_at']);
        ...
        return response(['success' => true, 'data' => $orders , 'storage' => $storage]);
    }
    ...
    public function apiCreateOrder(Request $request)
    {
        ...
        $request->validate([
            ...
            'user_id' => 'required_if:delivery_type,HOME',
            'date' => 'bail|required',
            'time' => 'bail|required',
        ...
        $data['order_status'] = 'incoming';
        $vendor = Vendor::where('user_id',auth()->user()->id)->first();
     
        $data['vendor_id'] = $vendor->id;
         $data['order_id'] = '#' . rand(100000, 999999);

        $order = Storage::create($data);
        ...
        foreach ($data['item'] as $item)
        {
            ...
            switch($item['package_type']) {
                   case "sp":  {$admin_commission = $admin_commission + (( ((int) $vendor->sp)/100) * $settings->vendor_tax_sp); break;}
                    case "mp":  {$admin_commission = $admin_commission + ((((int) $vendor->mp)/100) * $settings->vendor_tax_mp); break;}

    case "lp":  {$admin_commission = $admin_commission + ((((int) $vendor->lp)/100) * $settings->vendor_tax_mp); break;}
    ...
    default: { break;}
        ...
        $order->amount=$data['amount']+$admin_commission;
        $order->admin_commission=$admin_commission;
        $order->vendor_amount=$data['amount'];
        $order->save();
        if($data['delivery_type'] == 'HOME'){
             $settle = array();
        $settle['vendor_id'] = $order->vendor_id;
        $settle['order_id'] = $order->id;
        $settle['payment'] = $data['payment_status'];
        $settle['status'] = 0;
        $settle['vendor_status'] = 0;
        $settle['admin_earning'] = $admin_commission;
        $settle['vendor_earning'] = $data['amount'];
        Settle::create($settle);
        }

          if($data['delivery_type'] == 'STORAGE'){
              return response(['success' => true , 'data' => "Items send to Storage successfully wait for Delivery"]);
          }else{
    ...
    public function apiCreateUser(Request $request)
    {
        ...
        $user =new User();
         $data = $request->all();
        if (User::where('phone','=', intval($data['phone']))->exists()) {
            $user = User::where('phone', '=', intval($data['phone']))->first();
       ...
       $data['vendor_id'] = $vendor->id;
        ...
        $data['is_verified'] = 1;
        $data['email_id'] = $data['phone'].'@transporter-sudan.com';
    ...
    public function apiChangeStatus(Request $request)
    {
        ...
        if ($order) {
            ...
            if($order->delivery_type == 'STORAGE')
            {
