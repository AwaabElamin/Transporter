...
Future<BaseModel<Users>> getUsers() async {
  Users response;
  try {
    response = await ApiClient(ApiHeader().dioData()).getUsers();
...
Future<BaseModel<User>> createUser({required String name, required int phone}) async {
  User response;
  try {
    response = await ApiClient(ApiHeader().dioData()).createUser(name, phone);
  } catch (error, stacktrace) {
    print("Exception occur: $error stackTrace: $stacktrace");
    return BaseModel()..setException(ServerError.withError(error: error));
  }
...
class Users {
  ...
  Users({bool? success, List<Data>? data, String? msg}) {
    _success = success;
    _data = data;
    _msg = msg;
  }
  ...
  Users.fromJson(dynamic json) {
    ...
    _data = List<Data>.from(json["data"].map((x) => Data.fromJson(x)));
